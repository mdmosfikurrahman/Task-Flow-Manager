...
public class UsersServiceImpl(IUsersRepository repository) : IUsersService
{
    ...
    public async Task<UsersResponse> Create(UsersRequest request)
    {
        ...
        var entity = request.ToEntity<UsersRequest, Users>();
        var saved = await repository.SaveAsync(entity);
        return saved.ToResponse<Users, UsersResponse>();
    }
