...
public static class GeneralUtils
{
    ...
    /// <summary>
    /// Serializes the specified data object to JSON, encrypts the JSON using AES encryption with the provided key,
    /// and writes the encrypted content to a file on disk.
    /// </summary>
    /// <typeparam name="T">The type of the object to serialize.</typeparam>
    /// <param name="directoryPath">The full directory path where the encrypted file should be saved.</param>
    /// <param name="fileName">The name of the file to create, including the file extension (e.g., "data.enc").</param>
    /// <param name="data">The object to serialize and encrypt. Must not be null.</param>
    /// A string used to generate the AES encryption key. The same key must be used to decrypt the file later.
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    /// <exception cref="ArgumentException">
    /// Thrown if <paramref name="directoryPath"/>, <paramref name="fileName"/> is null or empty.
    /// </exception>
    /// <exception cref="ArgumentNullException">
    /// Thrown if <paramref name="data"/> is null.
    /// </exception>
    /// <example>
    /// await WriteEncryptedJsonFileAsync("C:\\secure", "data.enc", myModelInstance, "mySecretKey123");
    /// </example>
    public static async Task WriteEncryptedJsonFileAsync<T>(
        ...
        T data)
    {
        ...
        var encryptedJson = EncryptJson(json);
    ...
    /// <summary>
    /// Reads an encrypted JSON file from disk, decrypts its contents using the provided AES key, 
    /// and deserializes it into a strongly-typed object of type <typeparamref name="T"/>.
    /// </summary>
    /// <typeparam name="T">The type into which the decrypted JSON content should be deserialized.</typeparam>
    /// <param name="directoryPath">The full directory path where the encrypted JSON file is located.</param>
    /// <param name="fileName">The name of the encrypted JSON file (including extension).</param>
    /// <returns>
    /// A deserialized object of type <typeparamref name="T"/> if the file exists and contains valid encrypted JSON; 
    /// otherwise, returns <c>default</c>.
    /// </returns>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="directoryPath"/>, <paramref name="fileName"/> is null or empty.
    /// </exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown if the file exists but is empty or cannot be read, or if decryption fails due to invalid format or key mismatch.
    /// </exception>
    /// <example>
    /// var result = await ReadEncryptedJsonFileAsync&lt;MyModel&gt;("C:\\secure", "data.enc", "myStrongKey123");
    /// </example>
    public static async Task<T?> ReadEncryptedJsonFileAsync<T>(
        string directoryPath,
        string fileName)
    {
        ...
        var encryptedData = await File.ReadAllTextAsync(fullPath);
        if (string.IsNullOrWhiteSpace(encryptedData))
            throw new InvalidOperationException($"The file '{fullPath}' is empty or unreadable.");

        var decryptedJson = DecryptJson(encryptedData);
